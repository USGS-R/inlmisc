...
#'    Specify \code{"array"} for array data (such as heads) and
#'    \code{"flow"} for flow data (such as cell-by-cell flows).
#' @param rm.totim.0 logical.
#'    If true, stress-periods at time zero are removed.
#'
#' @details This function reads head, drawdown,
#'   and cell budget files generated by MODFLOW.
#'
...
#'       The format typically coincides with the horizontal model grid.
#'       The one exception is when \code{data.type = "flow"} and the
#'       cell-by-cell budget file was saved using the "COMPACT BUDGET" output option.
#'       The compact format stores values associated with each cell:
#'       columns include cell index (\code{"icell"}), model layer (\code{"layer"}),
#'       model row (\code{"row"}), model column (\code{"column"}), cell-by-cell flow (\code{"flow"}),
#'       and any auxiliary variables saved using the "AUXILIARY" output option.}
#'     \item{kstp}{time step}
#'     \item{kper}{stress period}
#'     \item{desc}{data-type description}
    ...
    if (data.type == "array") {
      ...
      if (nlay > 0) {
        ...
        for (i in seq_len(nlay)) {
          lst[[length(lst) + 1]] <- list(d=x[[i]], kstp=kstp, kper=kper, desc=desc,
                                         ilay=i, delt=as.numeric(NA),
                                         pertim=as.numeric(NA),
                                         totim=as.numeric(NA))
        }

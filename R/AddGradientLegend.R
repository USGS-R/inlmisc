...
#'
#' @examples
#' plot(NA, xlim = c(0, 100), ylim = c(-10, 10), xlab = "x", ylab = "y")
#' breaks <- 0:200
...
#' AddGradientLegend(breaks, rainbow, loc = "bottomright",
#'                   inset = c(0.2, 0.1), strip.dim = c(1, 14))
#'

AddGradientLegend <- function(breaks, pal, at=NULL, labels=TRUE, title=NULL,
  ...
  if (is.logical(labels) && labels) labels <- formatC(at, big.mark=",")

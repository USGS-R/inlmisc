...
#'   Determines the type of interval to be drawn.
#' @param col,lty,lwd,xpd graphical parameters; see \code{\link{par}}.
...
#'   \code{code = 1}, an interval head is drawn at \code{(x[i], y0[i])};
#'   \code{code = 2}, an interval head is drawn at \code{(x[i], y1[i])}; and
...
AddIntervals <- function(x, y0, y1, length=0.03, code=3,
  ...
  if (code %in% c(1L, 3L))
    graphics::segments(m[, 1], y0, x1=m[, 2],
                       col=col, lty=lty, lwd=lwd, xpd=xpd, lend=0)
  if (code %in% c(2L, 3L))
    graphics::segments(m[, 1], y1, x1=m[, 2],
                       col=col, lty=lty, lwd=lwd, xpd=xpd, lend=0)
